...
export default function CollectionScreen() {
  ...
  const playBirdSound = async (audioUrl: string | null | undefined, commonName: string) => {
    if (soundRef.current) {
    ...
    if (!audioUrl) {
      Alert.alert("Sem gravação", `Não há áudio salvo para ${commonName}.`);
      return;
    }
  ...
  const renderSightingItem = ({ item }: { item: Sighting }) => (
    <TouchableOpacity style={styles.sightingCard} activeOpacity={0.7}>
      <TouchableOpacity
        ...
        onPress={() => playBirdSound(item.audioUrl, item.commonName)}
        activeOpacity={0.7}
      >
