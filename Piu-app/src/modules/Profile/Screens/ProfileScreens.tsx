...
import { getMyBirdRecords, type BirdRecord } from "../../Collection/services/collectionService";

const SETTINGS_KEY = "passaralhos_settings";
...
const NOTE_BG_COLORS = ["#D1FAE5", "#FEF9C3", "#DBEAFE", "#FCE7F3", "#F3F4F6"];

function mapRecordToFieldNote(record: BirdRecord, index: number): FieldNote {
  return {
    id: record.id,
    commonName: record.common_name,
    scientificName: record.scientific_name,
    timeAgo: formatTimeAgo(record.created_at),
    location: record.location || "PIU",
    iconName: "feather",
    imageBg: NOTE_BG_COLORS[index % NOTE_BG_COLORS.length],
  };
}
...
export default function ProfileScreen() {
  ...
  const [recentNotes, setRecentNotes] = useState<FieldNote[]>(FIELD_NOTES);
  ...
  useEffect(() => {
    AsyncStorage.getItem(SETTINGS_KEY)
      .then((stored) => { if (stored) setSettings(JSON.parse(stored)); })
      .catch(() => {});
  }, []);
  ...
  const loadProfile = useCallback(async () => {
    try {
      ...
      const [data, recordsData] = await Promise.all([
        getMyProfile(),
        getMyBirdRecords().catch(() => ({ records: [], total: 0 })),
      ]);
      ...
      if (recordsData.records.length > 0) {
        ...
        setRecentNotes(sorted.slice(0, 3).map(mapRecordToFieldNote));
      }
    } catch (error) {
